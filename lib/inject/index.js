...
module.exports = exports = function (sources, opt) {
	if (!sources) {
		throw new Error('Missing sources stream!');
	}
	if (!opt) {
		opt = {};
	}
	...
	if (typeof opt.read !== 'undefined') {
		throw new Error('There is no `read` option. Did you mean to provide it for `gulp.src` perhaps?');
	}
	...
	throw new Error('passing target file as a string is deprecated! Pass a vinyl file stream (i.e. use `gulp.src`)!');
};
...
function handleVinylStream(sources, opt) {
	...
	return through2.obj(function (target, enc, cb) {
		var self = this;
		if (target.isStream()) {
			return cb(new Error('Streams not supported for target templates!'));
		}
		collected
...
function getNewContent(target, collection, opt) {
	var logger = opt.quiet ? noop : function (filesCount) {
		if (filesCount) {
			console.log(filesCount + ' files into ' + target.relative);
		} else {
			console.log('Nothing to inject into ' + target.relative);
		}
	};
...
function getTagsToInject(files, target, opt) {
	return files.reduce(function (lines, file, i, files) {
