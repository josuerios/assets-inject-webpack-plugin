...
module.exports = function getFilepath(sourceFile, targetFile, opt) {
	opt = opt || {};
...
function addRootSlash(filepath) {
	return filepath.replace(/^\/*([^\/])/, '/$1'); //eslint-disable-line
}
...
function removeBasePath(basedirs, filepath) {
	return basedirs.map(unixify)
		.reduce(function (path, remove) {
			if (path[0] === '/' && remove[0] !== '/') {
				remove = '/' + remove;
			}
			if (path[0] !== '/' && remove[0] === '/') {
